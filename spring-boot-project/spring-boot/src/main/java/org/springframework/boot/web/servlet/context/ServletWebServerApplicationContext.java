...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	/**
	 * 触发父类AbstractApplicationContext#refresh()接口，在refresh()中有一个模板接口onRefresh()接口
	 *
	 * @Author: xiaocainiaoya
	 * @Date: 2021/08/25 15:53:41
	 * @param
	 * @return:
	 **/
	@Override
	...
	/**
	 * spring framework中的扩展点, 在spirngboot中用于内嵌容器的处理
	 *
	 * @Author: xiaocainiaoya
	 * @Date: 2021/08/25 15:55:50
	 * @param
	 * @return:
	 **/
	@Override
	...
	protected void finishRefresh() {
		...
		if (webServer != null) {
			...
			// 启动成功, 发布ServletWebServerInitializedEvent事件
	...
	private void createWebServer() {
		...
		if (webServer == null && servletContext == null) {
			// 获取内嵌ServletWebServlet工厂
			ServletWebServerFactory factory = getWebServerFactory();

			// 根据嵌入式ServletWebServer工厂获取ServletWeb服务
			// 这时初始化了dispatcherServlet
		...
		// 初始化PropertySources, 用于使用ServletContext和ServletConfig代替ApplicationContext的Eventment中的这个两个属性源
		initPropertySources();
	}
	...
	protected ServletWebServerFactory getWebServerFactory() {
		...
		// 获取BeanFactory中ServletWebServerFactory类型的beanNames, 搜索不包括父BeanFactory
	...
	private void selfInitialize(ServletContext servletContext) throws ServletException {
		// 准备ApplicationContext 互换ServletContext和ApplicationContext中的一些属性
		prepareWebApplicationContext(servletContext);

		// 注册作用域, 包括request、session、application3种
		registerApplicationScope(servletContext);
		// 把servletContext和servletConfig注册为到springbean容器中
		WebApplicationContextUtils.registerEnvironmentBeans(getBeanFactory(), servletContext);
		// 调用所有初始化器的onstartup方法
		// DispatcherServlet就是在这里注册到tomcat容器中 -> DispathcerServletRegistrationBean
	...
	/**
	 * 将ServletContextScope注册到springbean容器和servlet容器中
	 *
	 * @Author: xiaocainiaoya
	 * @Date: 2021/08/26 10:37:12
	...
	/**
	 * 获取所有的ServletContextInitializer集合类
	 *
